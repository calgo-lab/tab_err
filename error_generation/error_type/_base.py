...
class ErrorType(ABC):
    ...
    @classmethod
    # TODO (seja): def apply(cls: type[ErrorType], table: pd.DataFrame, error_mask: pd.DataFrame, preserve_dtypes: bool = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    # 1. prüft parameters
    #   - table.shape == error_mask.shape
    # 2. kopiert 'table'
